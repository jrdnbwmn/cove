class AccountInvitationsController < ApplicationController
  ...
  def update
    if (account_user = @account_invitation.accept!(current_user))
      redirect_to account_path(account_user.account)
    else
      redirect_to account_invitation_path(@account_invitation), alert: @account_invitation.errors.full_messages.first || t("something_went_wrong")
    end
  end
